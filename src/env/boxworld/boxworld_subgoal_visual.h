// File: boxworld_subgoal.h
// Description: Subgoal boxworld_cpp environment

#ifndef HPTS_ENV_BOXWORLD_SUBGOAL_VISUAL_H_
#define HPTS_ENV_BOXWORLD_SUBGOAL_VISUAL_H_
...
class BoxWorldSubgoalVisualState : public BoxWorldBaseState {
public:
    ...
    friend auto operator<<(std::ostream &os, const BoxWorldSubgoalVisualState &state) -> std::ostream &;

    inline static const std::string name{"boxworld_subgoal_visual"};
...
namespace std {
template <>
struct hash<hpts::env::bw::BoxWorldSubgoalVisualState> {
    size_t operator()(const hpts::env::bw::BoxWorldSubgoalVisualState &state) const {
        return state.get_hash();
    }
};
}    // namespace std

#endif    // HPTS_ENV_BOXWORLD_SUBGOAL_VISUAL_H_
