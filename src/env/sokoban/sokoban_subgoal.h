// File: sokoban_subgoal.h
// Description: Subgoal sokoban_cpp environment

#ifndef HPTS_ENV_SOKOBAN_SUBGOAL_H_
#define HPTS_ENV_SOKOBAN_SUBGOAL_H_
...
class SokobanSubgoalState : public SokobanBaseState {
public:
    using SokobanBaseState::SokobanBaseState;
    ...
    friend auto operator<<(std::ostream &os, const SokobanSubgoalState &state) -> std::ostream &;

    [[nodiscard]] auto to_image() const -> std::vector<uint8_t>;

    inline static const std::string name{"sokoban_subgoal"};
    inline static const int num_actions = 4;
    inline static const int num_subgoals = 4 * 4;

private:
    auto get_subgoal_observation(std::size_t subgoal, bool remove_agent) const noexcept -> Observation;
...
namespace std {
template <>
struct hash<hpts::env::sokoban::SokobanSubgoalState> {
    size_t operator()(const hpts::env::sokoban::SokobanSubgoalState &state) const {
        return state.get_hash();
    }
};
}    // namespace std

#endif    // HPTS_ENV_SOKOBAN_SUBGOAL_H_
