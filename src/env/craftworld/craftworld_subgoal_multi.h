// File: craftworld_subgoal_multi.h
// Description: craftworld environment with generic multi subgoals

#ifndef HPTS_ENV_CRAFTWORLD_SUBGOAL_MULTI_H_
#define HPTS_ENV_CRAFTWORLD_SUBGOAL_MULTI_H_
...
class CraftWorldSubgoalMultiState : public CraftWorldSubgoalBaseState {
public:
    ...
    friend auto operator<<(std::ostream &os, const CraftWorldSubgoalMultiState &state) -> std::ostream &;

    inline static const std::string name{"craftworld_subgoal_multi"};
};
...
namespace std {
template <>
struct hash<hpts::env::cw::CraftWorldSubgoalMultiState> {
    size_t operator()(const hpts::env::cw::CraftWorldSubgoalMultiState &state) const {
        return state.get_hash();
    }
};
}    // namespace std

#endif    // HPTS_ENV_CRAFTWORLD_SUBGOAL_MULTI_H_
